...
export default function FloatingHelp({ show, user }) {
  ...
  // Anchors to the right edge of the centered 480px column instead of the raw viewport,
  // so it stays visible even on wide desktop windows.
  const anchorRight = "max(16px, calc(50vw - 240px + 16px))";
  return (
    <>
      {open && (
        <div style={{ position:"fixed", bottom:92, right:anchorRight, background:"#fff", borderRadius:18, padding:"16px", width:235, boxShadow:"0 10px 40px #0004", zIndex:1000, fontFamily:"'Poppins',sans-serif", animation:"fadeIn .2s ease", border:"1px solid #f0f0f0" }}>
